...
class TcpDslTest {
    ...
    void tcpStringThrowsForNullName() {
        assertThrows(NullPointerException.class, () -> TcpDsl.tcp(null));
    }
    ...
    void withLengthHeaderPrependsFourByteBigEndianHeader() {
        ...
        byte[] result = TcpDsl.withLengthHeader(message, TcpRequestBuilder.LengthHeaderType.FOUR_BYTE_BIG_ENDIAN);
    ...
    void withLengthHeaderPrependsTwoByteLittleEndianHeader() {
        byte[] message = {0x01};
        byte[] result = TcpDsl.withLengthHeader(message, TcpRequestBuilder.LengthHeaderType.TWO_BYTE_LITTLE_ENDIAN);
    ...
    void withLengthHeaderPrependsFourByteLittleEndianHeader() {
        byte[] message = {0x01, 0x02, 0x03};
        byte[] result = TcpDsl.withLengthHeader(message, TcpRequestBuilder.LengthHeaderType.FOUR_BYTE_LITTLE_ENDIAN);
